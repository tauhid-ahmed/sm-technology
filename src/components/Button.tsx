...
type Tone = "default" | "outline-1" | "outline-2" | "link";
...
const buttonVariantClasses: Record<Variant, Record<Tone, string>> = {
  primary: {
    ...
    "outline-2":
      "border border-color-gray-50 text-color-black hover:bg-color-black hover:text-white",
...
const buttonSizeClasses: Record<Size, string> = {
  sm: "px-4 py-2 text-sm rounded-md h-[2.8125rem]",
  lg: "px-6 py-3 text-base rounded-lg h-[3.3125rem]",
};
...
export default function Button({
  ...
  return (
    <Comp
      className={cn(
        "font-rubik inline-flex items-center justify-center transition-[transform_opacity] focus:outline-none focus:ring-2 focus:ring-offset-1 tracking-custom cursor-pointer disabled:cursor-not-allowed disabled:opacity-50 disabled:shadow-none disabled:hover:bg-transparent disabled:hover:text-inherit duration-300 hover:-translate-y-px gap-7 [&>svg]:size-4 [&>svg]:fill-current",
